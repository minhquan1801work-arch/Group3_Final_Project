...
| Dáng mặt phù hợp | ✅ (kính) | 1–2 giá trị: `tron`, `trai_xoan` (=Oval), `trai_tim` (=Mắt Mèo), `kim_cuong`, `vuong` | tron, trai_xoan |
| **Hình dáng gọng** (frameShape) | ✅ (kính) | `tron` / `oval` / `mat_meo` / `vuong` — **khác với dáng mặt!** Đây là hình dáng của chính chiếc kính | vuong |
| **Chất liệu gọng** (material) | ✅ (kính) | `nhua` / `kim_loai` | nhua |
| **Loại phụ kiện** (accessoryType) | ✅ (phụ kiện) | `hop_dung` / `khan_lau` / `nuoc_lau` / `tui_dung` / `day_deo` | khan_lau |
...
> **Phân biệt 2 loại shape** (quan trọng, hay nhầm):
> - `faceShapes` (dáng mặt phù hợp) = kính này hợp với người có KHUÔN MẶT dạng nào — dùng cho "Chọn kính theo dáng mặt" ở trang chủ
> - `frameShape` (hình dáng gọng) = bản thân chiếc kính có HÌNH gì — dùng cho filter "Shape Kính" ở trang sản phẩm
...
products/{autoId} = {
  ...
  faceShapes:  ["tron", "trai_xoan"],   // dáng KHUÔN MẶT phù hợp
  frameShape:  "vuong",                  // hình dáng GỌNG: tron|oval|mat_meo|vuong
  material:    "nhua",                   // nhua | kim_loai
  accessoryType: "khan_lau",             // CHỈ phụ kiện: hop_dung|khan_lau|nuoc_lau|tui_dung|day_deo
  gender:      "unisex",
