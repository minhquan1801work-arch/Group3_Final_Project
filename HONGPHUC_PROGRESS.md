...
### Thử kính ảo (AR try-on) — mục 2 SPEC_CAMERA_TRYON.md (12/07/2026)
- User cung cấp ảnh gọng kính demo (`24664-7-glasses-clipart.png`, 2261x676, RGBA nền trong suốt, thẳng mặt) — copy vào `res/drawable/glasses_tryon_demo.png`, đã verify bằng script kiểm tra alpha channel trước khi dùng.
- `build.gradle.kts`: thêm CameraX (`camera-core/camera2/lifecycle/view` 1.3.4) + ML Kit Face Detection (`face-detection:16.1.6`).
- `res/drawable/bg_pill_translucent_dark.xml` mới — nền pill mờ tối cho hint text đè trên camera preview.
- `activity_try_on.xml` mới — `PreviewView` (fillCenter) + `FaceOverlayView` overlay + nút back + hint text.
- `utils/FaceOverlayView.java` mới — custom View vẽ `glasses_tryon_demo.png` đè lên mặt: scale theo khoảng cách 2 mắt (tỉ lệ cố định `GLASSES_TO_EYE_DISTANCE_RATIO = 2.3f`, ước lượng cho ảnh demo dùng chung mọi SP), xoay theo `headEulerAngleZ` (roll), dùng logic quy đổi tọa độ ảnh phân tích → view kiểu center-crop (theo pattern chuẩn của Google ML Kit sample `GraphicOverlay`, có xử lý mirror cho camera trước).
- `ui/catalog/TryOnActivity.java` mới — xin quyền CAMERA runtime, CameraX bind `Preview` + `ImageAnalysis` (lens trước), chạy ML Kit Face Detection (`LANDMARK_MODE_ALL`, `PERFORMANCE_MODE_FAST`) trên từng frame lấy landmark `LEFT_EYE`/`RIGHT_EYE` → gọi `FaceOverlayView.updateFace()`.
- `AndroidManifest.xml`: khai báo `TryOnActivity` (`screenOrientation="portrait"`).
- `ProductDetailActivity.java`: `btnTryOn` đổi từ Toast placeholder sang `TryOnActivity.start(this)`.
- Build compileDebugJavaWithJavac PASS.
- **⚠️ CHƯA TEST TRÊN MÁY THẬT** (không có cách chạy camera live trong session này) — đặc biệt cần kiểm tra: (1) chiều xoay overlay theo góc nghiêng đầu (`rotation = imageFlipped ? -rollDegrees : rollDegrees` — dấu (+/-) chọn theo suy luận lý thuyết, **có thể cần đảo dấu** sau khi thấy thực tế trên máy), (2) tỉ lệ `GLASSES_TO_EYE_DISTANCE_RATIO` có thể cần chỉnh lại theo ảnh demo thật, (3) FPS/độ mượt của `ImageAnalysis` trên thiết bị tầm trung.
- Chưa làm (theo spec, không bắt buộc MVP): nút chụp lại ảnh kết quả.
- **Fix cảnh báo 16KB page size (12/07/2026)**: Android Studio báo `libimage_processing_util_jni.so` (native lib trong CameraX 1.3.4) chưa align 16KB — nâng cấp cả 4 lib CameraX lên `1.4.1`, verify lại bằng cách extract `.so` từ APK debug + đọc ELF program header xác nhận `PT_LOAD align = 16384` (16KB) — đã đúng, hết cảnh báo. Không ảnh hưởng máy hiện tại (chưa dùng kernel 16KB) nhưng nâng cấp cho sạch, tránh vướng khi nộp Play Store sau này.
- **Test máy thật + fix chiều xoay overlay (12/07/2026)**: User test AR try-on trên máy thật gửi ảnh chụp — nhìn thẳng/máy thẳng OK, nhưng nghiêng đầu trái thì kính xoay ngược sang phải (đúng như dự đoán trước, dấu rotation sai). Fix `FaceOverlayView.java`: đảo dấu `rotation = imageFlipped ? rollDegrees : -rollDegrees` (trước đó `imageFlipped ? -rollDegrees : rollDegrees`) — camera trước (mirror) cần giữ nguyên dấu `rollDegrees`, ngược lại lý thuyết ban đầu suy đoán. Build PASS, **cần user test lại trên máy để xác nhận chiều xoay đã đúng**.

### Đa ngôn ngữ (i18n) — Auth/Cart/Checkout/Payment (12/07/2026)
- Phát hiện: `rowLanguage` trong Settings trước đây chỉ mở `Settings.ACTION_LOCALE_SETTINGS` (cài đặt ngôn ngữ hệ thống Android), không tự đổi ngôn ngữ app — và app chỉ có `values/strings.xml` tiếng Việt, phần lớn UI hardcode thẳng text (không qua string resource) nên dù đổi ngôn ngữ máy cũng không có tác dụng.
- **Quyết định cùng user**: KHÔNG làm i18n toàn app (quá lớn so với thời gian còn lại) — chỉ scope 4 luồng quan trọng: **Auth (Welcome/Login/Signup/Forgot), Cart, Checkout (+ CheckoutVoucher), Payment**. Các màn xem sản phẩm/duyệt hàng giữ nguyên tiếng Việt.
- Đổi cơ chế: từ "theo ngôn ngữ hệ thống" sang **toggle chọn tay trong app** (Tiếng Việt/English) — dùng `AppCompatDelegate.setApplicationLocales()` (API chuẩn Android, project sẵn `appcompat 1.7.1` đủ điều kiện, không cần thêm dependency).
- Kéo ~150 chuỗi hardcode (8 file Java + 13 file layout) ra `strings.xml`, gồm cả lỗi Firebase Auth trong `AuthRepository.java` (đổi signature `login/register/loginWithGoogle/forgotPassword` thêm tham số `Context` để gọi `context.getString()`), tên màu hex→text trong `CartFragment.colorNameOf()`/`CheckoutActivity.displayColorName()`, toàn bộ voucher title/logic trong `CheckoutVoucherActivity`.
- Tạo `res/values-en/strings.xml` dịch đầy đủ (~230 dòng) cho toàn bộ key liên quan 4 luồng — các string khác của app (không có trong `values-en`) tự fallback về tiếng Việt mặc định, không lỗi.
- Quy ước: **giữ nguyên ký hiệu tiền tệ "đ"/"VND"** không dịch (giống các app thương mại điện tử khác — số tiền không đổi theo ngôn ngữ UI).
- `SettingsActivity.java`: `rowLanguage` giờ hiện `AlertDialog` chọn Tiếng Việt/English thay vì mở cài đặt hệ thống.
